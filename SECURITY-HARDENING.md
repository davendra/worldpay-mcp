...
- `npm test` — **22/22 tests pass** (the original 5, plus 17 new tests covering the SSRF/credential guard, log redaction, and schema rejection of bad amounts, non-ISO currencies, raw PANs, and path-traversal ids).
- MCP Inspector `tools/list` — exactly 9 tools, each carrying the new annotations; server version reports `1.2.0` (from `package.json`).
